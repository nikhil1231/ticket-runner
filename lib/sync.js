...
async function applyOp({ store, tracker, ticket, op }) {
  ...
  if (op.op === 'promote_incubator') {
    await tracker.promoteIncubator(ticket, op.payload.targetDatabaseId || op.payload.targetTrackerId || op.payload.targetProjectKey);
    return;
  }
  if (op.op === 'archive') {
    // Trackers that cannot archive (e.g. Notion today) simply skip it; the op
    // completes rather than parking so the ticket's remaining ops still flow.
    if (tracker.archiveItem) await tracker.archiveItem(ticket);
    return;
  }
