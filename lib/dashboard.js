...
const { normalizeProject } = require('./projects');
...
// heartbeat file. It never talks to GitHub, so it renders even when no
...
const DASHBOARD_SERVICES = [
  { id: 'ticket-runner', label: 'Ticket runner', unit: 'ticket-runner.service' },
  { id: 'dashboard', label: 'Dashboard', unit: 'ticket-runner-dashboard.service' },
];
...
// Build project views from config without touching GitHub.
function loadProjects(config) {
  ...
  const projects = raw.filter(Boolean).map((p) => {
    const flywheel = { ...(config.flywheel || {}), ...(p.flywheel || {}) };
    return {
      key: p.key,
      repoPath: p.repoPath,
      ...
      notes: p.notes || '',
      flywheelEnabled: !!flywheel.enabled,
      flywheelContinuous: !!flywheel.continuous,
      flywheelMaxEpics: flywheel.maxEpics || null,
    };
  });
  return { projects, registryOnly: false };
}
...
function connectionStatus(config, projects) {
  return CONNECTIONS.map((c) => {
    const required = projects.some((p) => c.need(p));
    return {
      id: c.id,
      label: c.label,
      env: c.env,
...
function listTicketColumns() {
  return 'id, parent_id, short_id, project_key, title, kind, status, last_agent, attempts, branch, updated_at';
}

function structureSummary(ticket) {
...
function visibleTicket(alias = 'tickets') {
  return `COALESCE(json_extract(${alias}.meta, '$.remoteMissing'), 0) != 1`
    + ` AND COALESCE(json_extract(${alias}.meta, '$.archived'), 0) != 1`;
}
...
function collectStoreData(baseDir) {
  ...
  try {
    ...
    const byStatus = {};
    for (const r of all(`SELECT status, COUNT(*) n FROM tickets WHERE ${visibleTicket()} GROUP BY status`)) byStatus[r.status] = r.n;

    const perProjectRows = all(`SELECT project_key, status, COUNT(*) n FROM tickets WHERE ${visibleTicket()} GROUP BY project_key, status`);
    const projectStatus = {};
    ...
    const activity = all(
      `SELECT e.type, e.from_status, e.to_status, e.created_at,
       ...
       WHERE t.id IS NULL OR ${visibleTicket('t')}
    ...
    const usageTotal = {};
    for (const r of all(`SELECT last_agent, COUNT(*) n FROM tickets WHERE last_agent <> '' AND ${visibleTicket()} GROUP BY last_agent`)) {
      const p = providerOf(r.last_agent);
      if (p) usageTotal[p] = (usageTotal[p] || 0) + r.n;
    }
    const usageRecent = {};
    for (const r of all(`SELECT last_agent, COUNT(*) n FROM tickets WHERE last_agent <> '' AND updated_at >= ? AND ${visibleTicket()} GROUP BY last_agent`, dayAgo)) {
      const p = providerOf(r.last_agent);
    ...
    const structuralRows = all(
      `SELECT ${listTicketColumns()}
       FROM tickets WHERE kind IN ('mission','epic')
       AND status NOT IN ('done','cancelled')
       AND ${visibleTicket()}
       ORDER BY project_key, created_at, short_id`
    ).map(rowToListTicket);
    const structuralById = new Map(structuralRows.map((item) => [item.id, item]));
    const attachParentContext = (item) => {
      if (!item) return item;
      const parent = structuralById.get(item.parentId);
      if (parent?.kind === 'epic') {
        item.epic = structureSummary(parent);
        const mission = structuralById.get(parent.parentId);
        if (mission?.kind === 'mission') item.mission = structureSummary(mission);
      } else if (parent?.kind === 'mission') {
        item.mission = structureSummary(parent);
      }
      return item;
    };
    structuralRows.forEach(attachParentContext);
    const projectStructure = {};
    for (const item of structuralRows) {
      const bucket = (projectStructure[item.project] ||= { missions: [], epics: [] });
      if (item.kind === 'mission') bucket.missions.push(item);
      else if (item.kind === 'epic') bucket.epics.push(item);
    }

    const running = all(
      `SELECT ${listTicketColumns()}
       FROM tickets WHERE status = 'in_progress'
       AND kind NOT IN ('mission','epic')
       AND ${visibleTicket()}
       ORDER BY updated_at DESC LIMIT 12`
    ).map(rowToListTicket).map(attachParentContext);
    const testing = all(
      `SELECT ${listTicketColumns()}
       FROM tickets WHERE status = 'testing'
       AND ${visibleTicket()}
       ORDER BY updated_at DESC LIMIT 12`
    ).map(rowToListTicket).map(attachParentContext);
    const queued = all(
      `SELECT ${listTicketColumns()}
       FROM tickets WHERE status = 'queued'
       AND ${visibleTicket()}
       ORDER BY created_at, short_id LIMIT 8`
    ).map(rowToListTicket).map(attachParentContext);
    const reviewing = all(
      `SELECT ${listTicketColumns()}
       FROM tickets WHERE status = 'in_review'
       AND ${visibleTicket()}
       ORDER BY updated_at DESC LIMIT 8`
    ).map(rowToListTicket).map(attachParentContext);
    // Latest tracker comment the runner posted per ticket, from the outbox.
    // The dashboard is deliberately offline (no tracker tokens), so it cannot
    // read replies typed directly on GitHub — but when the runner parks
    // a ticket it always posts the reason as a comment, which makes the newest
    // outbox comment the latest issue comment for exactly the tickets surfaced
    // here, and the best available "what is actually wrong" text.
    const lastComments = new Map();
    for (const r of all(
      `SELECT o.ticket_id, o.payload FROM outbox o
       JOIN (SELECT ticket_id, MAX(id) mid FROM outbox WHERE op = 'comment' GROUP BY ticket_id) latest
         ON latest.mid = o.id`
    )) {
      lastComments.set(r.ticket_id, String(parseJson(r.payload, {}).text || '').slice(0, 400));
    }

    // Tickets parked for a human. The runner never retries needs_info on its
    // own, so these belong front and centre next to dependency blockages.
    const needsInfo = all(
      `SELECT id, short_id, project_key, title, kind, attempts, review_feedback, tracker_meta, updated_at
       FROM tickets WHERE status = 'needs_info'
       AND ${visibleTicket()}
       ORDER BY updated_at DESC LIMIT 12`
    ).map((r) => ({
      shortId: r.short_id,
      ...
      attempts: r.attempts,
      at: r.updated_at,
      note: (r.review_feedback || '').slice(0, 400),
      lastComment: lastComments.get(r.id) || '',
      url: safe(() => JSON.parse(r.tracker_meta).url, '') || '',
    }));

    // Dependency blockages. readyTickets() silently skips queued tickets whose
    // dependencies are not yet testing/done/cancelled, which makes a stalled
    // pipeline look like an idle one. Walk the dependency graph transitively and
    // surface only chains that bottom out at a human-wait status
    // (needs_info/in_review/failed) — queued/in_progress blockers clear
    // themselves, so routine ordering is not a blockage.
    const blockageRows = all(
      `WITH RECURSIVE chain(ticket_id, blocker_id) AS (
         SELECT d.ticket_id, d.depends_on_id
         FROM ticket_dependencies d JOIN tickets b ON b.id = d.depends_on_id
         WHERE b.status NOT IN ('testing','done','cancelled') AND ${visibleTicket('b')}
         UNION
         SELECT c.ticket_id, d.depends_on_id
         FROM chain c
         JOIN ticket_dependencies d ON d.ticket_id = c.blocker_id
         JOIN tickets b ON b.id = d.depends_on_id
         WHERE b.status NOT IN ('testing','done','cancelled') AND ${visibleTicket('b')}
       )
       SELECT t.short_id, t.project_key, t.title, t.kind, t.status, t.attempts, t.updated_at, t.created_at,
              b.id blocker_id, b.short_id blocker_short_id, b.project_key blocker_project, b.title blocker_title,
              b.status blocker_status, b.attempts blocker_attempts, b.updated_at blocker_updated_at,
              b.review_feedback blocker_note, b.tracker_meta blocker_tracker_meta
       FROM chain c
       JOIN tickets t ON t.id = c.ticket_id
       JOIN tickets b ON b.id = c.blocker_id
       WHERE t.status = 'queued' AND b.status IN ('needs_info','in_review','failed')
         AND ${visibleTicket('t')} AND ${visibleTicket('b')}
       ORDER BY t.created_at, t.short_id`
    );
    const blockedMap = new Map();
    const blockerMap = new Map();
    for (const r of blockageRows) {
      const blocked = blockedMap.get(r.short_id) || {
        shortId: r.short_id, project: r.project_key, title: r.title, kind: r.kind,
        status: r.status, attempts: r.attempts, at: r.updated_at, blockedBy: [],
      };
      if (!blocked.blockedBy.includes(r.blocker_short_id)) blocked.blockedBy.push(r.blocker_short_id);
      blockedMap.set(r.short_id, blocked);
      const blocker = blockerMap.get(r.blocker_short_id) || {
        shortId: r.blocker_short_id, project: r.blocker_project, title: r.blocker_title,
        status: r.blocker_status, attempts: r.blocker_attempts, at: r.blocker_updated_at,
        note: (r.blocker_note || '').slice(0, 400),
        lastComment: lastComments.get(r.blocker_id) || '',
        url: safe(() => JSON.parse(r.blocker_tracker_meta).url, '') || '',
        blocks: [],
      };
      if (!blocker.blocks.includes(r.short_id)) blocker.blocks.push(r.short_id);
      blockerMap.set(r.blocker_short_id, blocker);
    }
    const blockages = {
      blocked: [...blockedMap.values()],
      blockers: [...blockerMap.values()].sort((a, b) => b.blocks.length - a.blocks.length),
    };
    for (const item of queued) item.blocked = blockedMap.has(item.shortId);

    const projectFlow = all(
      `SELECT project_key,
              SUM(CASE WHEN status = 'queued' THEN 1 ELSE 0 END) queued,
              SUM(CASE WHEN status IN ('in_progress','testing') THEN 1 ELSE 0 END) moving,
              SUM(CASE WHEN status IN ('failed','needs_info') THEN 1 ELSE 0 END) blocked,
              MAX(updated_at) updated_at
       FROM tickets
       WHERE status IN ('queued','in_progress','testing','failed','needs_info')
       AND ${visibleTicket()}
       GROUP BY project_key
       HAVING queued > 0 OR moving > 0 OR blocked > 0
       ORDER BY moving DESC, blocked DESC, queued DESC, updated_at DESC
       LIMIT 8`
    ).map((r) => ({
      project: r.project_key,
      queued: r.queued || 0,
    ...
    const projectFlowByProject = Object.fromEntries(projectFlow.map((item) => [item.project, item]));
    const stackSummary = {
      deployed: stacks.filter((s) => s.status === 'deployed').length,
      blocked: stacks.filter((s) => s.status === 'blocked').length,
      other: stacks.filter((s) => s.status !== 'deployed' && s.status !== 'blocked').length,
    };

    const stats = safe(() => store.stats(), { tickets: 0, byStatus: {}, outboxPending: 0, outboxParked: 0 });
    const visibleTickets = one(`SELECT COUNT(*) AS n FROM tickets WHERE ${visibleTicket()}`)?.n || 0;
    const integrity = safe(() => one('PRAGMA integrity_check').integrity_check, 'unknown');

    return {
      totals: { tickets: visibleTickets, outboxPending: stats.outboxPending || 0, outboxParked: stats.outboxParked || 0 },
      byStatus, projectStatus, projectStructure, projectFlowByProject, completed, attention, activity, blockages, needsInfo,
      current: {
        running,
        testing,
        inFlight: running,
    ...
    return {
      ...
      projectStatus: {},
      projectStructure: {},
      projectFlowByProject: {},
      completed: [],
      attention: [],
      activity: [],
      blockages: { blocked: [], blockers: [] },
      needsInfo: [],
      current: { running: [], testing: [], inFlight: [], queued: [], reviewing: [], recentlyCompleted: [], projectFlow: [], stackSummary: { deployed: 0, blocked: 0, other: 0 } },
      usage: { total: {}, recent: {} },
      stacks: [],
...
function collectTicketDetails(baseDir, shortId) {
  ...
  try {
    ...
    if (!ticket) return null;
    if (ticket.meta?.remoteMissing || ticket.meta?.archived) return null;
    const all = (sql, ...args) => safe(() => db.prepare(sql).all(...args), []);
    ...
    const dependencies = all(
      `SELECT d.dep_type, t.short_id, t.title, t.status, t.project_key
       ...
       WHERE d.ticket_id = ? AND ${visibleTicket('t')} ORDER BY t.created_at, t.short_id`,
      ticket.id
    ).map((r) => ({
      type: r.dep_type,
      shortId: r.short_id,
    ...
    const blockedBy = all(
      `SELECT d.dep_type, t.short_id, t.title, t.status, t.project_key
       ...
       WHERE d.depends_on_id = ? AND ${visibleTicket('t')} ORDER BY t.created_at, t.short_id`,
      ticket.id
    ).map((r) => ({
      type: r.dep_type,
      shortId: r.short_id,
...
function clampLogLines(lines) {
  return Math.max(20, Math.min(1000, Number(lines) || 300));
}
...
// codex's CLI transcript alternates short narration ("codex"/"thinking" blocks,
// one bare line as a section marker) with tool calls ("exec") and file edits
// ("apply patch") whose raw body — grep/test/build output,a full unified
// diff — can run thousands of lines. That body is exactly the "firehose" a
// human glancing at live progress doesn't want; the narration is exactly what
// they do want. This is a structural (marker-line) heuristic, not an LLM call:
// it recognizes codex's specific transcript format and is a no-op on any
// other engine's output (agy/claude don't emit these bare-line markers), so
// it's safe to run unconditionally over both stdout and stderr.
const TRANSCRIPT_MARKER_RE = /^(codex|thinking|exec|apply patch|user)$/;
const DIFF_BODY_RE = /^(index [0-9a-f]|--- |\+\+\+ |@@ |[+\- ])/;

function summarizeTranscript(text) {
  const lines = text.split('\n');
  const out = [];
  let mode = null; // null (untouched) | 'narration' | 'exec' | 'patch' | 'user'
  let bodyLines = 0;
  // A unified-diff header ends up in the transcript two ways: inside an
  // explicit "apply patch" block,pasted verbatim into a "codex" text
  // response (models sometimes recap their own diff in the final summary,
  // despite the prompt asking them not to) — track it independent of `mode`
  // so both are collapsed the same way, everywhere.
  let inDiff = false;
  let lastDiffLine = '';

  const flushExec = () => {
    if (bodyLines > 2) out.push(`  … ${bodyLines - 2} line(s) of output collapsed`);
  };

  for (const line of lines) {
    if (TRANSCRIPT_MARKER_RE.test(line)) {
      if (mode === 'exec') flushExec();
      mode = line === 'codex' || line === 'thinking' ? 'narration' : line === 'exec' ? 'exec' : line === 'apply patch' ? 'patch' : 'user';
      bodyLines = 0;
      inDiff = false;
      lastDiffLine = '';
      if (mode === 'user') { out.push('user', '  … ticket prompt omitted (see ticket details)'); continue; }
      out.push(line);
      continue;
    }
    if (/^diff --git /.test(line)) {
      // codex emits one header per hunk, so the same file can repeat back to
      // back for a multi-hunk edit; collapse those immediate repeats.
      if (line !== lastDiffLine) out.push(line);
      lastDiffLine = line;
      inDiff = true;
      continue;
    }
    if (/^patch: /.test(line)) { out.push(line); inDiff = false; continue; }
    if (inDiff) {
      if (DIFF_BODY_RE.test(line)) continue; // index/---/+++/@@ and the +/-/context hunk body
      inDiff = false; // first line that isn't diff shape — e.g. a trailing "tokens used" report
    }

    if (mode === 'narration' || mode === null) { out.push(line); continue; }
    if (mode === 'user') continue; // drop the full re-echoed prompt
    if (mode === 'exec') {
      bodyLines += 1;
      if (bodyLines <= 2) out.push(line); // command line, then the "succeeded/failed in Xms:" status line
    }
    // mode === 'patch' outside a diff (e.g. the bare file-path echo line) — drop.
  }
  if (mode === 'exec') flushExec();
  return out.join('\n');
}

function readTailLines(file, lines, maxBytes = 512 * 1024) {
  try {
...
function fileUpdatedAt(files) {
  let mtimeMs = 0;
  for (const file of files) {
    try { mtimeMs = Math.max(mtimeMs, fs.statSync(file).mtimeMs); } catch {}
  }
  return mtimeMs ? new Date(mtimeMs).toISOString() : null;
}

function newestTaskRunDir(baseDir, shortId) {
  const runsDir = path.join(baseDir, 'runs');
  const id = String(shortId || '').trim();
  if (!id) return null;
  const runNameRe = new RegExp(`^${escapeRe(id)}(?:-\\d+|-review-\\d+|-plan-\\d+)$`);
  const runStamp = (name) => Number(name.match(/-(\d+)$/)?.[1] || 0);
  let entries = [];
  try { entries = fs.readdirSync(runsDir, { withFileTypes: true }); } catch { return null; }
  const matches = entries
    .filter((entry) => entry.isDirectory() && runNameRe.test(entry.name))
    .map((entry) => {
      const dir = path.join(runsDir, entry.name);
      if (!isPathInside(runsDir, dir)) return null;
      return {
        name: entry.name,
        dir,
        mtimeMs: safe(() => fs.statSync(dir).mtimeMs, 0),
      };
    })
    .filter(Boolean)
    .sort((a, b) => (b.mtimeMs - a.mtimeMs) || (runStamp(b.name) - runStamp(a.name)) || b.name.localeCompare(a.name));
  return matches[0] || null;
}

function invocationDirs(runDir) {
  const out = [];
  const record = (dir, tag) => {
    const stdout = path.join(dir, 'stdout.log');
    const stderr = path.join(dir, 'stderr.log');
    if (!fs.existsSync(stdout) && !fs.existsSync(stderr)) return;
    out.push({ dir, tag, stdout, stderr });
  };
  let entries = [];
  ...
  for (const entry of entries) {
    if (entry.isDirectory()) {
      ...
      if (isPathInside(runDir, dir)) record(dir, entry.name);
    }
  }
  if (!out.length) record(runDir, path.basename(runDir));
  return out.sort((a, b) => a.tag.localeCompare(b.tag, undefined, { numeric: true }));
}

function collectTaskLogs(baseDir, shortId, { lines = 300 } = {}) {
  const count = clampLogLines(lines);
  const db = openDb(baseDir);
  try {
    const row = safe(() => db.prepare(`SELECT ${listTicketColumns()} FROM tickets WHERE short_id = ? AND ${visibleTicket()}`).get(shortId), null);
    const ticket = row ? rowToListTicket(row) : null;
    const run = newestTaskRunDir(baseDir, shortId);
    const invocations = run ? invocationDirs(run.dir).map((inv) => ({
      tag: inv.tag,
      stdoutLines: readTailLines(inv.stdout, count),
      stderrLines: readTailLines(inv.stderr, count),
      updatedAt: fileUpdatedAt([inv.stdout, inv.stderr]),
    })) : [];
    return {
      ticket,
      run: run ? { name: run.name, updatedAt: run.mtimeMs ? new Date(run.mtimeMs).toISOString() : null } : null,
      invocations,
...
function dashboardCodeVersion(baseDir) {
  const files = [
    'runner.js',
    path.join('lib', 'dashboard.js'),
    path.join('dashboard', 'index.html'),
    path.join('dashboard', 'src', 'main.jsx'),
    path.join('dashboard', 'src', 'styles.css'),
    path.join('public', 'dashboard', 'index.html'),
  ];
  const assetsDir = path.join(baseDir, 'public', 'dashboard', 'assets');
  for (const entry of safe(() => fs.readdirSync(assetsDir, { withFileTypes: true }), [])) {
    if (entry.isFile()) files.push(path.join('public', 'dashboard', 'assets', entry.name));
  }
  const parts = [headSha(baseDir)];
  for (const rel of files) {
    const file = path.join(baseDir, rel);
    parts.push(`${rel}:${safe(() => fs.statSync(file).mtimeMs, 0)}`);
  }
  return parts.join('|');
}
...
function defaultRestartCommand() {
  return process.platform === 'win32'
    ? 'restart unavailable on Windows; stop and rerun npm run dashboard'
    : 'systemctl --user restart ticket-runner.target';
}

function collectData(config, { baseDir, serverInfo = {} }) {
  ...
  return {
    ...
    dashboard: {
      codeVersion: dashboardCodeVersion(baseDir),
      url: serverInfo.url || '',
      ...
      checkout: baseDir,
      restartCommand: serverInfo.restartCommand || defaultRestartCommand(),
    },
    ...
    projects,
    registryOnly,
    services: DASHBOARD_SERVICES.map(({ id, label }) => ({ id, label })),
    providers,
    connections,
    store,
    tokens,
...
function collectServiceLogs(serviceId, { lines = 200, execute = execFileSync } = {}) {
  const service = DASHBOARD_SERVICES.find((item) => item.id === serviceId);
  if (!service) {
    const error = new Error('unknown service');
    error.statusCode = 404;
    throw error;
  }
  const count = Math.max(20, Math.min(1000, Number(lines) || 200));
  if (process.platform === 'win32') {
    const error = new Error('service logs are available from systemd journal on the deployed host');
    error.statusCode = 503;
    throw error;
  }
  if (!commandOnPath('journalctl')) {
    const error = new Error('journalctl is not available on PATH');
    error.statusCode = 503;
    throw error;
  }
  const text = execute('journalctl', ['--user', '-u', service.unit, '-n', String(count), '--no-pager', '-o', 'short-iso'], {
    encoding: 'utf8',
    timeout: 5000,
    maxBuffer: 1024 * 1024,
  ...
  return {
    service: { id: service.id, label: service.label, unit: service.unit },
    generatedAt: new Date().toISOString(),
...
<p>The API is running, but the React dashboard has not been built yet. Run <code>npm run dashboard:build</code> from the ticket-runner checkout,wait for the next auto-update build.</p>
...
function startServer(config, { baseDir, port = 4600, host = '127.0.0.1', restart = restartRunnerService, logReader = collectServiceLogs } = {}) {
  const serverInfo = {
    host,
    port,
    url: `http://${host}:${port}`,
  ...
  const server = http.createServer((req, res) => {
    ...
    if (url === '/api/data') {
      let body;
      try {
        body = JSON.stringify(collectData(config, { baseDir, serverInfo }));
      } catch (error) {
    ...
    if (logsMatch) {
      try {
        const details = logReader(decodeURIComponent(logsMatch[1]), { lines: parsedUrl.searchParams.get('lines') });
    ...
    const taskLogsMatch = url.match(/^\/api\/task-logs\/([^/]+)$/);
    if (taskLogsMatch) {
      try {
        const details = collectTaskLogs(baseDir, decodeURIComponent(taskLogsMatch[1]), { lines: parsedUrl.searchParams.get('lines') });
        sendJson(res, 200, details);
      } catch (error) {
  ...
  return new Promise((resolve) => {
    server.listen(port, host, () => {
      ...
      serverInfo.port = typeof address === 'object' && address ? address.port : port;
      serverInfo.url = `http://${host}:${serverInfo.port}`;
      resolve({ server, url: serverInfo.url });
    });
  });
}
...
function render(d){
  ...
  if(!s.attention.length)h+='<div class="empty">Nothing waiting — no failed, in-review,needs-info tickets.</div>';
  else{
...
function reviewHistoryTable(items){
  if(!items||!items.length)return '<div class="empty">No review findings recorded yet.</div>';
  let h='<div class="tblwrap"><table><thead><tr><th>When</th><th>Round</th><th>Reviewer</th><th>Finding</th></tr></thead><tbody>';
  for(const e of items)h+='<tr><td class="num">'+ago(e.at)+'</td><td class="num">'+valueOrDash(e.round)+'</td><td class="mono">'+valueOrDash(e.reviewer)+'</td><td>'+esc(e.notes||'')+'</td></tr>';
  return h+'</tbody></table></div>';
}
...
function renderTicketDetails(d){
  ...
  h+='<div class="detail-grid">'+
    field('Project',t.projectKey)+field('Kind',t.kind)+field('Attempts',t.attempts)+field('Review rounds',t.reviewRounds)+field('Requeues',t.requeueCount)+
  ...
  h+='<h2>Review history (all-time, never cleared)</h2>'+reviewHistoryTable(t.reviewHistory);
...
module.exports = { collectData, collectTicketDetails, collectTaskLogs, collectServiceLogs, restartRunnerService, startServer, summarizeTranscript };
