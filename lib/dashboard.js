...
function collectStoreData(baseDir) {
  ...
  try {
    ...
    const inFlight = all(
      `SELECT short_id, project_key, title, kind, status, last_agent, attempts, branch, updated_at
       FROM tickets WHERE status IN ('in_progress','testing')
    ...
    return {
      ...
      attention: [],
      activity: [],
      current: { inFlight: [], queued: [], reviewing: [], recentlyCompleted: [], projectFlow: [], stackSummary: { deployed: 0, blocked: 0, other: 0 } },
      usage: { total: {}, recent: {} },
      stacks: [],
...
module.exports = { collectData, collectTicketDetails, collectServiceLogs, restartRunnerService, startServer };
