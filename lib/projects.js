...
function normalizeProject(config, raw) {
  ...
  if (!raw.databaseId) throw new Error(`${key}: ticket database ID is required`);
  ...
  return {
    ...
    tracker: normalizeTracker(raw.tracker, raw.databaseId),
  };
}
...
async function resolveProjects(config, notion) {
  ...
  const projects = Array.isArray(config.projects) && config.projects.length
    ? config.projects.map((project) => normalizeProject(config, project))
    : (config.boards || []).map((board) => legacyProjectFromBoard(config, board));
