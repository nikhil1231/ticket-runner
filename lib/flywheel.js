...
async function runPlannerAgent({ config, board, log, services, prompt, tag, key, max, allowComplete = false }) {
...
async function runEpicPhase({ config, board, store, log, services, mission, epics, settings }) {
  ...
  const prompt = buildEpicPrompt({ mission, board, epics: displayEpics });
...
async function runTicketPhase({ config, board, store, log, services, mission, epic, settings }) {
  ...
  const fallback = await runPlannerAgent({ config, board, log, services, prompt, tag: `epic-${epic.shortId}`, key: 'tickets', max, allowComplete: true });
  if (fallback.status === 'stop') {
