...
const FLYWHEEL_DEFAULTS = {
  ...
  // completing every epic idles the mission until a human edits itadds an
  // epic by hand.
  continuous: false,
  backlogThreshold: 2,
  maxOpenTickets: 10,
  // Standing size of the In-review epic pool: the flywheel keeps this many
  // proposals awaiting human pick at once, refilling as they are approved or
  // rejected (see runFlywheelPass). Also caps how many it proposes per round.
  maxEpics: 7,
  maxTicketsPerPass: 3,
...
function missionIsPerpetual(mission) {
  if (mission?.kind !== 'mission') return false;
  const tags = [
    ...(mission.trackerMeta?.tags || []),
    ...(mission.trackerMeta?.labels || []),
    ...(mission.meta?.tags || []),
  ];
...
function trackerKeyFor(board) {
  ...
  throw new Error(`unsupported tracker type: ${board.tracker?.type || 'missing'}`);
}
...
- Read project conventions such as CLAUDE.md, AGENTS.md, docs,README files when present.
...
- Do not repeat existingrejected epics.
...
\`\`\`
- dependsOn holds zero-based indexes of earlier epics in this same list that must land first; omitleave empty if none.`;
}
...
# Recently completedfailed tickets (context, do not duplicate)
${recentList}
${notes}
# Instructions
...
// ---- phases ----
async function runEpicPhase({ config, board, store, log, services, mission, epics, settings, slots = settings.maxEpics }) {
  ...
  const prompt = buildEpicPrompt({ mission, board, epics: displayEpics, max: slots });
  const fallback = await runPlannerAgent({ config, board, log, services, prompt, tag: 'epics', key: 'epics', max: slots, policy: plannerPolicy(config, 'epicPlanner') });
  ...
  const kept = dedupeAgainst(fallback.value.items, existingTitles).slice(0, slots);
  ...
  if (created.length) {
    store.enqueueComment(mission.id, `Flywheel proposed ${created.length} epic(s) for review: ${created.map((epic) => epic.title).join(', ')}. Move each to Not started to approve,Cancelled to reject.`);
  }
...
async function runTicketPhase({ config, board, store, log, services, mission, epic, settings }) {
  ...
  // The planner reports there is nothing new worth adding to the epic. This is a
  // sticky decision, but "nothing more to add" is not "everything is finished":
  // tickets already created may still be queuedin progress. If every ticket
  // is already mergedcancelled, close the epic outright and rotate. If every
  // ticket has at least reached the testing branch, park it in Testing for human
  // sign-off (Done cascades a merge of all tickets; In progress resumes). But if
  // any ticket is still being worked (queued/in_progress/in_review/needs_info),
  // leave the epic In progress and wait — reconcileEpics runs at the top of every
  // pass and parks it in Testing on its own once the last ticket lands, so the
  // epic never gets declared ready while its own planned work is still queued.
  if (fallback.value.status === 'complete') {
    ...
    const allInTestingStack = children.every((child) => ['testing', 'done', 'cancelled'].includes(child.status));
    if (!allInTestingStack) {
      const stillWorking = children.filter((child) => isOpen(child) && child.status !== 'testing').length;
      store.enqueueComment(epic.id, `Flywheel has no more tickets to add here${reason ? `: ${reason}` : ''}, but ${stillWorking} ticket(s) are still being worked. Keeping the epic In progress; it will move to Testing once every ticket reaches the testing branch.`);
      return { status: 'epic_waiting' };
    }
    store.transition(epic.id, 'testing');
    const testingCount = children.filter((child) => child.status === 'testing').length;
    store.enqueueComment(epic.id, `Flywheel has no more tickets to add here${reason ? `: ${reason}` : ''}. Moved the epic to Testing (${testingCount} ticket(s) in the testing branch). Move it to Done to merge every ticket to main at once,back to In progress to keep adding tickets.`);
    if (epic.parentId) store.enqueueComment(epic.parentId, `Epic "${epic.title}" is ready for sign-off${reason ? ` (${reason})` : ''}.`);
    return { status: 'epic_testing' };
  }
...
function reconcileEpics({ store, board, log = () => {}, continuous = false }) {
  ...
  for (const epic of epics) {
    ...
    const allTerminal = children.every((child) => isSettled(child)); // done/cancelled
    const anyDone = children.some((child) => child.status === 'done');
    // Every ticket has reached the testing brancha terminal state (none is
    // still being worked), and at least one actually landed (not all cancelled).
    const allInTestingStack = children.every((child) => ['testing', 'done', 'cancelled'].includes(child.status));
    const anyLive = children.some((child) => ['testing', 'done'].includes(child.status));
    if (allTerminal && anyDone) {
      // Nothing is parked in the testing branch waiting to cascade — every ticket
      // is already merged to main (or cancelled). Close the epic outright.
      store.transition(epic.id, 'done');
      store.enqueueComment(epic.id, `All ${children.length} ticket(s) under this epic are merged. Marking the epic done.`);
      if (epic.parentId) {
    ...
    } else if (epic.status === 'in_progress' && allInTestingStack && anyLive) {
      // All tickets are complete and in the testing branch: park the epic in
      // Testing for human sign-off rather than merging automatically. Moving the
      // epic to Done cascades a merge of every ticket to main; moving it back to
      // In progress resumes ticket generation.
      const testingCount = children.filter((child) => child.status === 'testing').length;
      store.transition(epic.id, 'testing');
      store.enqueueComment(epic.id, `All ${children.length} ticket(s) under this epic are complete and in the testing branch. Moved the epic to Testing. Move it to Done to merge every ticket to main at once,back to In progress to keep adding tickets.`);
      if (epic.parentId) store.enqueueComment(epic.parentId, `Epic "${epic.title}" is ready for sign-off (${testingCount} ticket(s) in testing).`);
      log(`flywheel: epic "${epic.title}" moved to Testing for sign-off (${projectKey})`);
    }
  }
  ...
  for (const mission of missions) {
    ...
    // Continuous projects and mission-level Perpetual tags both regenerate
    // epics instead of treating the mission as finished.
    if (continuous || missionIsPerpetual(mission)) continue;
    ...
    store.enqueueComment(mission.id, 'All epics under this mission are completerejected. Update the mission body to propose new epics,add one manually.');
    store.setKv(idleHashKey, currentIdleHash);
  }
}

// ---- orchestration ----
function rotateAfter(items, lastId) {
  if (!items.length || !lastId) return items;
  const index = items.findIndex((item) => item.id === lastId);
  if (index < 0) return items;
  return [...items.slice(index + 1), ...items.slice(0, index + 1)];
}

function flywheelProgressed(result) {
  return result.status === 'epic_complete'
    || result.status === 'epic_testing'
    || (result.status === 'ok' && result.created > 0);
}

function flywheelShouldContinue(result) {
  return ['cooldown', 'awaiting_epic_approval', 'mission_idle', 'backlog_full', 'open_ticket_cap'].includes(result.status);
}

async function runMissionFlywheelPass({ config, board, store, log, services, mission, settings }) {
  const projectKey = board.key || board.app;
  const cooldownKey = `flywheel:cooldown:${projectKey}:${mission.id}`;
  const failuresKey = `flywheel:failures:${projectKey}:${mission.id}`;
  const cooldownUntil = store.getKv(cooldownKey, null);
  if (cooldownUntil && cooldownUntil > new Date().toISOString()) return { status: 'cooldown', missionId: mission.id };

  const missionContinuous = settings.continuous || missionIsPerpetual(mission);

  // A mission the flywheel is actively decomposing belongs In progress, not
  // parked in Not started - that column is the queue the runner pulls unstarted
  // tickets from, not where live work sits.
  if (mission.status === 'queued') mission = store.transition(mission.id, 'in_progress');
  ...
  // Epic proposals are a semaphore, not a latch: keep a standing pool of
  // `maxEpics` proposals In review at all times so a human always has that many
  // to pick from. Propose enough to refill the pool whenever it has drained
  // below maxEpics (a prior proposal was approved or rejected) rather than
  // waiting for every epic to settle first. A one-shot mission still only
  // proposes on the first run or after a mission edit - it does not keep
  // refilling - so its epics stay a fixed batch; a continuous mission tops the
  // pool back up indefinitely.
  const inReviewCount = epics.filter((epic) => epic.status === 'in_review').length;
  const proposalSlots = Math.max(0, settings.maxEpics - inReviewCount);
  const noEpicsYet = epics.length === 0;
  const allEpicsSettled = epics.length > 0 && openEpics.length === 0;
  const shouldProposeEpics = proposalSlots > 0
    && (noEpicsYet || missionChanged || missionContinuous);

  try {
    let result;
    if (shouldProposeEpics) {
      result = await runEpicPhase({ config, board, store, log, services, mission, epics, settings, slots: proposalSlots });
      store.setKv(hashKey, currentHash);
    } else {
      // The actively-worked epic lives In progress. Approval only moves an epic
      // to Not started (queued); the flywheel promotes the oldest such epic to In
      // progress and works it there, so a worked epic never sits parked in Not
      // started. Epics still In review,already parked in Testing awaiting
      // sign-off, are left untouched here.
      const byAge = (a, b) => String(a.createdAt || '').localeCompare(String(b.createdAt || ''));
      let targetEpic = epics.filter((epic) => epic.status === 'in_progress').sort(byAge)[0];
      if (!targetEpic) {
        const nextApproved = epics.filter((epic) => epic.status === 'queued').sort(byAge)[0];
        if (nextApproved) {
          targetEpic = store.transition(nextApproved.id, 'in_progress');
          store.enqueueComment(targetEpic.id, 'Flywheel started work on this approved epic; moved it to In progress.');
          log(`flywheel: promoted approved epic "${targetEpic.title}" to In progress (${projectKey})`);
        }
      }
      const allFeatures = store.ticketsByKind(projectKey, 'feature');
    ...
    log(`flywheel pass failed for ${projectKey} / ${mission.shortId} (${classification.kind}): ${error.message}`);
    try {
      store.enqueueComment(mission.id, `Flywheel pass failed for ${projectKey}; will retry after a cooldown.

${error.message}`);
    } catch {}
    return { status: 'error', error: error.message, missionId: mission.id };
  }
}
...
async function runFlywheelPass({ config, board, store, log = () => {}, services = {} } = {}) {
  ...
  const missions = store.ticketsByKind(projectKey, 'mission')
    ...
    .sort((a, b) => String(a.createdAt || '').localeCompare(String(b.createdAt || '')) || String(a.shortId || '').localeCompare(String(b.shortId || '')));
  if (!missions.length) return { status: 'no_mission' };

  const lastMissionKey = `flywheel:last-mission:${projectKey}`;
  const ordered = rotateAfter(missions, store.getKv(lastMissionKey, null));
  let fallback = null;
  for (const mission of ordered) {
    const result = await runMissionFlywheelPass({ config, board, store, log, services, mission, settings });
    if (!fallback || (fallback.status === 'cooldown' && result.status !== 'cooldown')) fallback = result;
    if (flywheelShouldContinue(result)) continue;
    store.setKv(lastMissionKey, mission.id);
    return result;
  }
  return fallback || { status: 'no_mission' };
}
