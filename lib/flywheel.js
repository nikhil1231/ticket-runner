...
async function runFlywheelPass({ config, board, store, log = () => {}, services = {} } = {}) {
  ...
  if (mission.status === 'queued') store.transition(mission.id, 'in_progress');
  ...
  try {
    ...
    // Parking an epic in Testing (epic_testing) or closing it (epic_complete) is
    // forward progress: clear the cooldown so the next approved epic starts
    // promptly instead of waiting out a needs-info-style backoff.
