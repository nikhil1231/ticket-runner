...
async function runTicketPhase({ config, board, store, log, services, mission, epic, settings }) {
  ...
  // tickets already created may still be queued or in progress. If every ticket
  // is already merged or cancelled, close the epic outright and rotate. If every
  ...
  if (fallback.value.status === 'complete') {
    ...
    store.enqueueComment(epic.id, `Flywheel has no more tickets to add here${reason ? `: ${reason}` : ''}. Moved the epic to Testing (${testingCount} ticket(s) in the testing branch). Move it to Done to merge every ticket to main at once, or back to In progress to keep adding tickets.`);
...
function reconcileEpics({ store, board, log = () => {}, continuous = false }) {
  ...
  for (const epic of epics) {
    ...
    // Every ticket has reached the testing branch or a terminal state (none is
    ...
    if (allTerminal && anyDone) {
      ...
      const testingCount = children.filter((child) => child.status === 'testing').length;
      store.transition(epic.id, 'testing');
      store.enqueueComment(epic.id, `All ${children.length} ticket(s) under this epic are complete and in the testing branch. Moved the epic to Testing. Move it to Done to merge every ticket to main at once, or back to In progress to keep adding tickets.`);
...
async function runMissionFlywheelPass({ config, board, store, log, services, mission, settings }) {
  ...
  try {
    ...
    if (shouldProposeEpics) {
      ...
      // started. Epics still In review, or already parked in Testing awaiting
