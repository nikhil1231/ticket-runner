...
async function runTicketPhase({ config, board, store, log, services, mission, epic, settings }) {
  ...
  // The planner reports the epic's scope is fully delivered. This is a sticky
  // decision: freeze the epic so the flywheel stops re-asking (it only works
  // In-progress epics). If nothing is in the testing branch to cascade-merge —
  // every ticket already merged or cancelled — close the epic outright and
  // rotate. Otherwise park it in Testing for human sign-off: the human moves it
  // to Done to merge every ticket at once, or back to In progress to resume.
  if (fallback.value.status === 'complete') {
    ...
    store.enqueueComment(epic.id, `Flywheel has no more tickets to add here${reason ? `: ${reason}` : ''}. Moved the epic to Testing (${testingCount} ticket(s) in the testing branch${stillWorking ? `, ${stillWorking} still finishing` : ''}). Move it to Done to merge every ticket to main at once, or back to In progress to keep adding tickets.`);
