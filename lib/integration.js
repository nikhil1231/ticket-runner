...
async function testingEntries({ config, board, notion = notionDefault, extraTicket = null, log = console.log, parkInvalid = true }) {
  ...
  for (const ticket of byId.values()) {
    ...
    if (nativeFiles.length) {
      if (parkInvalid) await park(notion, ticket, `📱 This ticket changes stack-blocked files and requires human testing before it can enter the cumulative stack.\n\n${nativeFiles.join('\n')}`, log);
      continue;
    }
...
async function admitTicket(args) {
  ...
  if (nativeFiles.length) {
    await park(notion, args.ticket, `📱 This ticket changes stack-blocked files and requires human testing before it can enter the cumulative stack.\n\n${nativeFiles.join('\n')}`, log);
