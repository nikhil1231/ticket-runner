...
async function park(tracker, ticket, message, log) {
  await tracker.mirror(ticket, { status: 'in_review' });
  await tracker.comment(ticket, message);
}

async function testingEntries({ config, board, tracker, extraTicket = null, log = console.log, parkInvalid = true }) {
  tracker = tracker || getProjectTracker(board, { log });
  const tickets = await tracker.listByStatus('Testing');
  ...
  for (const ticket of byId.values()) {
    ...
    if (nativeFiles.length) {
      if (parkInvalid) await park(tracker, ticket, `📱 This ticket changes stack-blocked files and requires human testing before it can enter the cumulative stack.\n\n${nativeFiles.join('\n')}`, log);
      continue;
    }
...
async function reconcileBoard({ config, board, tracker, eas = easDefault, log = console.log, extraTicket = null, force = false, services = {} }) {
  tracker = tracker || getProjectTracker(board, { log });
  const settings = integrationSettings(config, board);
  ...
  let entries = await testingEntries({ config, board, tracker, extraTicket, log });
  const previous = state.readState(config.baseDir, stateKey(board), null);
  ...
  while (true) {
    ...
    await park(tracker, ticket, `⚠ This ticket conflicted while composing the cumulative testing stack and was removed for human review.\n\nConflicting files:\n${conflict.files.join('\n') || '(Git did not report file names)'}`, log);
  ...
  state.writeState(config.baseDir, stateKey(board), deployed);
  return deployed;
}
...
async function admitTicket(args) {
  ...
  if (nativeFiles.length) {
    await park(tracker, args.ticket, `📱 This ticket changes stack-blocked files and requires human testing before it can enter the cumulative stack.\n\n${nativeFiles.join('\n')}`, log);
  ...
  const result = await reconcileBoard({ ...args, tracker, extraTicket: args.ticket, force: true });
...
async function promoteTicket({ config, board, ticket, tracker, log = console.log, services = {} }) {
  ...
  try {
    try {
      ...
      if (!files.length) throw error;
      await park(tracker, ticket, `⚠ Automatic merge conflicted with the latest ${settings.mainBranch}. The ticket was returned to In review.\n\nConflicting files:\n${files.join('\n') || '(Git did not report file names)'}`, log);
    ...
    try {
      ...
      await park(tracker, ticket, `⚠ Automatic merge validation failed. The ticket was returned to In review.\n\n${error.message.slice(-1800)}`, log);
...
async function stackStatus({ config, board, tracker, log = () => {} }) {
  ...
  return {
    ...
    projectKey: projectKey(board),
    desired,
    deployed: state.readState(config.baseDir, stateKey(board), null),
  };
}
