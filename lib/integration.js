...
// The Testing column holds both implemented tickets awaiting stack inclusion and
// planning artifacts parked for human sign-off (an epic frozen in Testing once
// its scope is delivered). Only the former have branch/commit metadata, so the
// testing stack is composed of implementable kinds; epics and missions are
// excluded (otherwise the stack reconciler parks them for "missing metadata").
const STACK_KINDS = new Set(['feature', 'query', 'incubator']);
const inStack = (ticket) => !ticket.kind || STACK_KINDS.has(ticket.kind);
...
// Replay only a ticket's own commits (baseSha..headSha) onto whatever `dir` is
// currently at, stripping any ancestor commits the head carried from the stack it
// was built on. Returns true if the replay applied cleanly, false (and cleans up
// the aborted cherry-pick) if it conflicted or produced nothing.
function applyOwnRange(dir, baseSha, headSha) {
  try {
    git(dir, ['cherry-pick', `${baseSha}..${headSha}`]);
    return true;
  } catch {
    try { git(dir, ['cherry-pick', '--abort']); } catch {}
    return false;
  }
}

// When a stacked head is merged onto a promotion base and the result is soft-reset
// to that base, the staged diff can include files a *different* stacked predecessor
// contributed (a ticket built on a composite that also held unrelated work). Restore
// any staged file that none of the promoted `entries` actually changed back to the
// base, so only the targeted tickets' own changes remain staged. No-op (and safe)
// when base metadata is unavailable. Returns the list of files it dropped.
function dropContamination(repo, dir, baseSha, entries) {
  if (!entries.length || entries.some(({ meta }) => !meta.baseSha)) return [];
  const ownFiles = new Set(entries.flatMap(({ meta }) =>
    worktrees.changedFilesBetween(repo, meta.baseSha, meta.headSha)));
  const staged = git(dir, ['diff', '--cached', '--name-only'])
    .split(/\r?\n/).map((file) => file.trim().replace(/\\/g, '/')).filter(Boolean);
  const contamination = staged.filter((file) => !ownFiles.has(file));
  // Restore each contaminated path to its base state — deleting the ones the base
  // doesn't have (new work a predecessor added). `git restore --source` handles the
  // absent-in-base case, which `git checkout <tree> -- <path>` would error on.
  if (contamination.length) git(dir, ['restore', `--source=${baseSha}`, '--staged', '--worktree', '--', ...contamination]);
...
async function testingEntries({ config, board, tracker, extraTicket = null, log = console.log, parkInvalid = true, allowNativeSensitive = false }) {
  ...
  const tickets = (store ? store.listByStatus(projectKey(board), 'testing') : await tracker.listByStatus('Testing')).filter(inStack);
  ...
  while (true) {
    ...
    for (const entry of entries) {
      try {
        ...
        try { git(built.dir, ['merge', '--abort']); } catch {}
        // Auto-heal before parking: the merge may have conflicted only because this
        // ticket's head still carries ancestor commits already present in the stack
        // (drift between the claim-time base and the current stack tip). Replaying
        // just the ticket's own commits strips those ancestors and resolves such
        // cases; a genuine same-region overlap re-conflicts and is parked below.
        // Only attempt it when the base really is an ancestor of the head — a head
        // on unrelated history is a broken state to surface, not to heal.
        if (entry.meta.baseSha
          && worktrees.isAncestor(repo, entry.meta.baseSha, entry.meta.headSha)
          && applyOwnRange(built.dir, entry.meta.baseSha, entry.meta.headSha)) {
          appliedEntries.push(entry);
          continue;
        }
...
async function promoteTicket({ config, board, ticket, tracker, log = console.log, services = {} }) {
  ...
  // A ticket implemented on top of the testing stack has a base that isn't on main
  // yet, so its head carries predecessor tickets as ancestors. Merging it wholesale
  // would drag those predecessors onto main. Detect that case by base ancestry (not
  // baseSha !== mainSha — a merely-old ticket has an ancestor base and must keep the
  // plain merge path so its head stays an ancestor of main).
  const stacked = meta.baseSha && !worktrees.isAncestor(repo, meta.baseSha, baseSha);
  ...
  try {
    if (stacked) {
      try {
        ...
        try { git(promotion.dir, ['merge', '--abort']); } catch {}
        if (!files.length) throw error;
        await park(tracker, ticket, `⚠ Automatic merge conflicted with the latest ${settings.mainBranch} (this ticket was built on other tickets not yet on ${settings.mainBranch}, or it genuinely conflicts). The ticket needs human intervention.\n\nConflicting files:\n${files.join('\n')}`, log);
        return { status: 'conflict', files };
      }
      // Collapse to only this ticket's own change: soft-reset to main stages the
      // whole merged range, then drop any files an unrelated stacked predecessor
      // brought in. If nothing remains staged, the change is already on main
      // (idempotent retry) — finalize without pushing.
      git(promotion.dir, ['reset', '--soft', baseSha]);
      dropContamination(repo, promotion.dir, baseSha, [{ meta }]);
      let squashSha = baseSha;
      if (git(promotion.dir, ['status', '--porcelain'])) {
        git(promotion.dir, ['commit', '-m', `${board.scope || projectKey(board)}: ${ticket.title}`]);
        try {
        ...
        squashSha = worktrees.head(promotion.dir);
        ...
        git(promotion.dir, ['push', settings.remote, `${squashSha}:refs/heads/${settings.mainBranch}`]);
      }
      await tracker.mirror(ticket, { status: 'done', forAI: false });
      await tracker.comment(ticket, `✅ Merged automatically to ${settings.mainBranch} (own changes replayed off the testing stack).\nMerge: ${squashSha}\nTicket commit: ${meta.headSha}`);
      worktrees.removeWorktree({ repoPath: repo, dir: meta.dir, branch: meta.branch, ignoreErrors: true });
      if (config.store && storeTicket?.id) config.store.clearWorktree(storeTicket.id);
      else ticketState.removeMeta(config.baseDir, ticket.shortId);
      return { status: squashSha === baseSha ? 'already_merged' : 'merged', mainSha: squashSha };
    }
    try {
      git(promotion.dir, ['merge', '--no-ff', '--no-edit', meta.headSha]);
    } catch (error) {
...
function squashMessage(board, epic, entries) {
  const scope = board.scope || projectKey(board);
  const lines = entries.map(({ ticket, meta }) => `- ${ticket.title} (${String(meta.headSha).slice(0, 12)})`);
  return `${scope}: ${epic.title}\n\nSquashed ${entries.length} ticket(s) from the testing stack:\n${lines.join('\n')}`;
}

// Moving an epic to Done means "land everything under this epic on main." Rather
// than promoting each ticket individually (one merge commit per ticket, every
// intermediate commit carried over as-is), collect all of the epic's Testing
// children, replay their heads onto the latest main in a throwaway branch, and
// collapse the whole range into a single squashed commit for the epic. Returns
// { status, merged: [ids...] }; 'remote_advanced' leaves the epic open to retry.
async function promoteEpic({ config, board, epic, children, tracker, log = console.log, services = {} }) {
  tracker = tracker || getProjectTracker(board, { log });
  const settings = integrationSettings(config, board);
  const repo = repoPath(config, board);
  const fetchBranch = services.fetchBranch || worktrees.fetchBranch;
  ...
  // Resolve each child's branch metadata, parking (and skipping) any whose
  // metadata or commit is missing — the same guardrails a single-ticket
  // promotion applies. The survivors are squashed together.
  const entries = [];
  for (const child of children) {
    const storeTicket = config.store && child.id ? config.store.getById(child.id) : child;
    const meta = config.store ? metaFromStoreTicket(storeTicket) : ticketState.readMeta(config.baseDir, child.shortId);
    if (!meta?.headSha || meta.pageId !== child.pageId || metaProjectKey(meta) !== projectKey(board)) {
      await park(tracker, child, '⚠ Automatic merge could not start because the ticket branch metadata is missing.', log);
      continue;
    }
    try {
      worktrees.commitRef(repo, meta.headSha);
    } catch {
      await park(tracker, child, '⚠ Automatic merge could not start because the recorded ticket commit no longer exists.', log);
      continue;
    }
    entries.push({ ticket: storeTicket || child, meta });
  }
  if (!entries.length) return { status: 'noop', merged: [] };

  const baseSha = fetchBranch(repo, settings.remote, settings.mainBranch);
  const promotion = promotionWorktree(config, { ...epic, projectKey: projectKey(board), repoPath: repo, project: board }, baseSha);
  try {
    for (const { ticket, meta } of entries) {
      try {
        git(promotion.dir, ['merge', '--no-ff', '--no-edit', meta.headSha]);
      } catch (error) {
        const files = conflictFiles(promotion.dir);
        try { git(promotion.dir, ['merge', '--abort']); } catch {}
        await park(tracker, ticket, `⚠ Squashing this epic's tickets to ${settings.mainBranch} conflicted on "${ticket.title}". The ticket needs human intervention.\n\nConflicting files:\n${files.join('\n') || '(Git did not report file names)'}`, log);
        return { status: 'conflict', files, ticket, merged: [] };
      }
    }

    // Collapse everything applied since main into a single staged change, then
    // one commit. If nothing is staged, every ticket's change is already on main
    // (e.g. a previous pass pushed the squash but crashed before finalizing) —
    // recover idempotently by finalizing the tickets without pushing again.
    git(promotion.dir, ['reset', '--soft', baseSha]);
    // Drop files an out-of-epic ticket contributed: a child stacked (via overlap
    // gating) on a composite that also held unrelated work would otherwise pull
    // that work into this epic's squash. Keep only the children's own changes.
    dropContamination(repo, promotion.dir, baseSha, entries);
    const alreadyMerged = !git(promotion.dir, ['status', '--porcelain']);
    let squashSha = baseSha;
    if (!alreadyMerged) {
      git(promotion.dir, ['commit', '-m', squashMessage(board, epic, entries)]);
      try {
        setup(promotion.dir, board, config.installTimeoutMs);
        validate(promotion.dir, settings.validationCommands, settings.validationTimeoutMs);
      } catch (error) {
        await tracker.comment(epic, `⚠ Squashing this epic to ${settings.mainBranch} failed validation. It needs human intervention.\n\n${String(error.message).slice(-1800)}`);
        return { status: 'validation_failed', error: error.message, merged: [] };
      }
      squashSha = worktrees.head(promotion.dir);
      const latest = fetchBranch(repo, settings.remote, settings.mainBranch);
      if (latest !== baseSha) return { status: 'remote_advanced', expected: baseSha, actual: latest, merged: [] };
      git(promotion.dir, ['push', settings.remote, `${squashSha}:refs/heads/${settings.mainBranch}`]);
    }
    ...
    for (const { ticket, meta } of entries) {
      await tracker.mirror(ticket, { status: 'done', forAI: false });
      await tracker.comment(ticket, `✅ Merged to ${settings.mainBranch} in the squashed epic "${epic.title}".\nSquash commit: ${squashSha}\nTicket commit: ${meta.headSha}`);
      worktrees.removeWorktree({ repoPath: repo, dir: meta.dir, branch: meta.branch, ignoreErrors: true });
      if (config.store && ticket.id) config.store.clearWorktree(ticket.id);
      else ticketState.removeMeta(config.baseDir, ticket.shortId);
      merged.push(ticket.id || ticket.pageId);
    }
    return { status: alreadyMerged ? 'already_merged' : 'merged', mainSha: squashSha, merged };
  } finally {
    removeGeneratedWorktree(config, promotion.dir, promotion.branch, board);
  }
}
...
async function stackStatus({ config, board, tracker, log = () => {} }) {
  tracker = tracker || getProjectTracker(board, { log });
  const tickets = (config.store ? config.store.listByStatus(projectKey(board), 'testing') : await tracker.listByStatus('Testing')).filter(inStack);
...
module.exports = {
  ...
  testingEntries, reconcileBoard, admitTicket, promoteTicket, promoteEpic, stackStatus,
};
