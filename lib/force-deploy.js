...
async function forceDeploy({ baseDir, board, ticket, notion, eas, log = console.log }) {
  ...
  if (meta.pageId !== ticket.pageId || meta.app !== board.app) {
  ...
  if (!board.easChannel) {
    const reason = `No EAS channel is configured for ${board.app}.`;
    await notion.safeComment(ticket.pageId, `⚠️ Force deploy could not start. ${reason}`, log);
    return { status: 'failed', reason };
  }

  log(`force deploying "${ticket.title}" from ${meta.branch} to "${board.easChannel}"`);
  const result = eas.pushUpdate({
    worktreeDir: meta.dir,
    appDir: board.appDir,
    channel: board.easChannel,
    message: `${board.scope}: ${ticket.title} [${meta.branch}] (human override)`,
    log,
  });

  if (!result.ok) {
    const reason = lastErrorLine(result.error);
  ...
  await notion.safeComment(ticket.pageId, `🚀 Force deployed by human override → Testing\nBranch: ${meta.branch}\nEAS channel: ${board.easChannel}`, log);
  return { status: 'deployed', branch: meta.branch, channel: board.easChannel };
}
