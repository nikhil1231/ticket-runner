...
const { relationIds } = require('./projects');
...
function extractIncubatorTicket(page) {
  const projectKey = (
    property(page, 'Project key')?.select?.name
    || (property(page, 'Project key')?.rich_text && richTextToPlain(property(page, 'Project key')?.rich_text))
    || property(page, 'Project')?.select?.name
    || property(page, 'App')?.select?.name
    || ''
  ).toLowerCase().trim();
  return {
    pageId: page.id,
    ...
    app: projectKey,
    projectKey,
    projectRelationIds: relationIds(page, 'Project'),
...
async function runIncubatorTicket({ config, board, ticket, log }) {
  ...
  try {
    ...
    if (config.selfHealing?.enabled !== false && (config.selfHealing?.maxRescuePasses ?? 1) > 0) {
      ...
      await notion.safeComment(ticket.pageId, `Running one bounded planning rescue pass with ${label(rescue)}.`, log);
      ...
      if (!result.timedOut && result.code === 0) {
        ...
        if (parsed.status === 'success') {
          await updateManagedPlan(ticket.pageId, existingPlan, parsed.plan);
          await notion.updatePage(ticket.pageId, {
            Status: { status: { name: 'In review' } },
            'Last agent': { rich_text: [{ text: { content: label(rescue).slice(0, 200) } }] },
          });
          await notion.safeComment(ticket.pageId, `Plan ready after rescue (${label(rescue)}).`, log);
          return { status: 'completed_after_rescue' };
        }
        if (parsed.status === 'needs_info') {
          await notion.updatePage(ticket.pageId, { Status: { status: { name: 'Needs info' } } });
          await notion.safeComment(ticket.pageId, parsed.message, log);
          return { status: 'needs_info' };
        }
      }
    }
    ...
    if (classification.kind === 'infrastructure') {
      if (classification.transient) {
        try {
          ...
          await notion.safeComment(ticket.pageId, `Transient planning infrastructure fault; requeued without consuming an attempt.\n\n${error.message}`, log);
        } catch (requeueError) {
          log(`transient planning failure could not update ticket: ${requeueError.message}`);
        }
        return { status: 'transient_requeued' };
      }
      await notion.safeComment(ticket.pageId, `Runner infrastructure fault detected during planning; attempting guarded self-healing.\n\n${error.message}`, log);
      const repaired = await repairRunner({ config, error, runDir, log });
      await notion.updatePage(ticket.pageId, {
        Status: { status: { name: repaired.status === 'deployed' ? 'Not started' : 'Failed' } },
        Attempts: { number: ticket.attempts },
      });
      await notion.safeComment(ticket.pageId, repaired.status === 'deployed'
        ? `Runner repair ${repaired.repairSha.slice(0, 7)} deployed; planning was requeued without consuming an attempt.`
        : `Self-healing stopped safely (${repaired.status}): ${repaired.reason || 'repair circuit is open'}.`, log);
...
async function handoffTicket({ config, ticket, board, log }) {
  if (!board) {
    await notion.updatePage(ticket.pageId, { Status: { status: { name: 'Needs info' } } });
    await notion.safeComment(ticket.pageId, 'Select a Project before moving this ticket to Done.', log);
    return false;
  }
  try {
    ...
    await notion.safeComment(ticket.pageId, `Handoff to ${board.key || board.app} failed: ${error.message}`, log);
    return false;
  }
}
