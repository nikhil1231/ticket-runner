...
function createGithubTracker({
  ...
  async function pollCommands({ store, projectKey }) {
    ...
    for (const raw of response.data || []) {
      ...
      const existing = store.getByTrackerId(`github:${owner}/${repo}`, String(item.number));
      const snap = snapshot(item, projectKey, status, `github:${owner}/${repo}`);
      if (!existing) commands.push({ type: 'create', trackerId: String(item.number), snapshot: snap });
        ...
        commands.push({ type: 'withdraw', trackerId: String(item.number), ticket: existing, snapshot: snap });
      } else if (status === 'cancelled' && !['done', 'failed', 'cancelled'].includes(existing.status)) {
        commands.push({ type: 'cancel', trackerId: String(item.number), ticket: existing, snapshot: snap });
      }
