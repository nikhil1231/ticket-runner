...
function createGithubTracker({
  ...
  async function pollCommands({ store, projectKey, kind }) {
    ...
    if (!notModified) {
      ...
      for (const raw of response.data || []) {
        ...
        if (kind && snap.kind !== kind) continue;
        if (!existing) {
    ...
    for (const [number, project] of projects) {
      ...
      if (status === 'done' && existing.kind === 'incubator') {
        commands.push({ type: 'incubator_approve', trackerId: String(number), ticket: existing, snapshot: snap });
