...
function createNotionTracker({ transport = notionDefault, databaseId, log = console.log } = {}) {
  ...
  async function pollCommands({ store, projectKey, kind = 'feature', extract = extractTicket } = {}) {
    ...
    for (const page of queuedPages) {
      ...
      const snapshot = snapshotFromTicket({ ...ticket, databaseId }, projectKey, kind);
      if (!existing) {
    ...
    for (const page of donePages) {
      ...
      else if (existing.status === 'testing') commands.push({ type: 'authorize_merge', trackerId: ticket.pageId, ticket: existing, snapshot });
  ...
  // Async mirror used by the sync engine: a Notion page always pre-exists (a
  // human created it), so this updates in place and reports the stable id back.
  async function upsertMirror(ticket, payload) {
    const id = pageOf(ticket);
    if (!id) throw new Error('cannot mirror a ticket with no Notion page id');
