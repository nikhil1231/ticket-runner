...
function snapshotFromTicket(ticket, projectKey, kind = 'feature') {
  ...
  if (ticket.tags?.length) snapshot.trackerMeta.tags = ticket.tags;
  return snapshot;
}
...
function createNotionTracker({ transport = notionDefault, databaseId, log = console.log } = {}) {
  ...
  async function pollCommands({ store, projectKey, kind = 'feature', extract = extractTicket } = {}) {
    ...
    for (const page of inProgressPages) {
      ...
      } else if (existing && hasPerpetualTag(existing.trackerMeta?.tags || []) !== hasPerpetualTag(ticket.tags || [])) {
        commands.push({ type: 'refresh_intent', trackerId: ticket.pageId, ticket: existing, snapshot: snapshotFromTicket({ ...ticket, databaseId }, projectKey, ticket.kind || kind) });
      }
    }
