...
function stackBaseForTestingDependencies({ store, ticket, board, repo }) {
  ...
  // 1) Explicit relationships: testing-status dependencies or same-epic siblings.
...
// latest note or '', so a ticket parked for a human and later re-queued had no
...
async function applyReviewOutcome({ config, store, tracker, board, ticket, implemented, branch, modelLabel, baseMirror, rev, log }) {
  ...
  if (rev.inconclusive || round > maxRounds) {
    ...
    const cycleNote = cycles >= 2
      ? `\n\nThis ticket has already bounced back to implementation ${cycles} time(s) after landing in Testing or being parked for review. Every distinct review finding across all attempts is preserved and will be given to the next implementation, so it shouldn't need rediscovering - but consider fixing it by hand or splitting the ticket if it keeps bouncing.`
      : '';
