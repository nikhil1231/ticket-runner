...
function stackBaseForTestingDependencies({ store, ticket, board }) {
  ...
  if (!testingDeps.length) return null;

  const stack = store.getStack(board.key || board.app);
  if (stack?.status !== 'deployed' || !stack.compositeSha) return null;
  const stacked = new Set((stack.tickets || []).flatMap((item) => [item.shortId, item.pageId].filter(Boolean)));
  if (!testingDeps.every((dep) => stacked.has(dep.shortId) || stacked.has(dep.pageId))) return null;
...
async function runTicket({ config, board, ticket, log, services = {} }) {
  ...
  try {
    ...
    ticketBaseRef = stackBaseForTestingDependencies({ store, ticket, board }) || mainRef;
...
module.exports = { runTicket, QUOTA_RE, buildPrompt, extractCommitMessage, compactAgentSummary, applyReviewOutcome };
