...
// requeue for another implementation round or, past maxRounds, park for a
// human. Pulled out of runTicket so it can be unit-tested without spinning up
// a real coding-agent engine.
//
...
async function applyReviewOutcome({ config, store, tracker, board, ticket, implemented, branch, modelLabel, baseMirror, rev, log }) {
  ...
  if (rev.inconclusive || round > maxRounds) {
    ...
    await tracker.mirror(ticket, { ...baseMirror, status: 'in_review', reviewRounds: 0, reviewFeedback: rev.notes || '' });
