...
const { blocksToMarkdown, richTextToPlain } = require('./ticket');
...
async function runTicket({ config, board, ticket, log, services = {} }) {
  ...
  let ticketBaseRef = config.baseBranch;
  ...
  async function parkInfrastructureFailure(error) {
    ...
    if (classification.transient) {
      try {
        await notion.updatePage(ticket.pageId, {
          ...
          Attempts: { number: originalAttempts },
        });
        await notion.safeComment(ticket.pageId, `Transient infrastructure fault; requeued without consuming an attempt.\n\n${truncate(error.message, 1200)}`, log);
      } catch (requeueError) {
    ...
    if (classification.kind === 'configuration' || classification.kind === 'user') {
      ...
      await notion.safeComment(ticket.pageId, `Configuration prevents this ticket from running; no ticket attempt was consumed.\n\n${truncate(error.message, 1800)}`, log);
      return { status: 'needs_info' };
    }

    await notion.safeComment(ticket.pageId, `🩺 Runner infrastructure fault detected; ticket attempt paused while guarded self-healing runs.\n\n${truncate(error.message, 1200)}\n\nLogs: ${runDir}`, log);
    ...
    if (repaired.status === 'deployed') {
      ...
      await notion.safeComment(ticket.pageId, `🩹 Runner repair ${repaired.repairSha.slice(0, 7)} passed validation and was deployed. This ticket was requeued without consuming an attempt.`, log);
    ...
    await notion.safeComment(ticket.pageId, `🛑 Self-healing stopped safely (${repaired.status}): ${repaired.reason || 'repair circuit is open'}. No ticket attempt was consumed.\n\nLogs: ${runDir}`, log);
  ...
  async function finalize(implemented, worktreeDir, branch, body) {
    ...
    const currentMeta = ticketState.readMeta(config.baseDir, ticket.shortId) || {};
    ...
    ticketState.writeMeta(config.baseDir, ticket.shortId, {
      ...
      headSha,
      changedFiles,
      nativeSensitiveFiles: integration.nativeSensitiveFiles(changedFiles, board),
    ...
    if (rev.verdict === 'approve') {
      const deployed = await integration.admitTicket({ config, board, ticket, notion, log });
      ...
      await notion.safeComment(ticket.pageId, `✅ Approved by ${reviewerLabel} → cumulative Testing stack\nEngine: ${modelLabel}\nBranch: ${branch}\nCommit: ${truncate(implemented.commits, 500)}\nStack: ${deployed.compositeSha || 'already current'}\n\nWhat changed\n${implemented.summary}\n\nReview\n${truncate(rev.notes || 'LGTM', 700)}`, log);
  ...
  try {
    const previousMeta = ticketState.readMeta(config.baseDir, ticket.shortId) || {};
    const [blocks, comments, bot] = await Promise.all([
      notion.getBlockChildren(ticket.pageId),
      notion.getComments(ticket.pageId),
      notion.getCurrentBot(),
    ]);
    const body = blocksToMarkdown(blocks);
    const processedCommentIds = new Set(previousMeta.processedCommentIds || []);
    const newHumanComments = comments
      .filter((comment) => comment.created_by?.id !== bot.id && !processedCommentIds.has(comment.id))
      .map((comment) => ({ id: comment.id, text: richTextToPlain(comment.rich_text).trim() }))
    ...
    if (isQueryOnlyTicket(ticket, body)) {
      return runQueryTicket({
        ...
        runDir,
        log,
        services: services.queryServices || {},
      });
    }
    ...
    ticketBaseRef = worktrees.fetchBranch(config.repoPath, settings.remote, settings.mainBranch);
    ...
    // One deliberately bounded rescue pass gets the complete failure context.
    // It targets the app worktree, never the runner repository.
    if (config.selfHealing?.enabled !== false && (config.selfHealing?.maxRescuePasses ?? 1) > 0) {
      ...
      await notion.safeComment(ticket.pageId, `🛟 Normal candidates were exhausted; running one bounded rescue pass with ${label(rescue)}.`, log);
      ...
      if (rescued.status === 'needs_info') {
        await notion.updatePage(ticket.pageId, { Status: { status: { name: 'Needs info' } } });
        await notion.safeComment(ticket.pageId, rescued.message, log);
        return { status: 'needs_info' };
      }
