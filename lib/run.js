...
const { buildChain, spawnEngine, QUOTA_RE, readTail } = require('./engine');
...
async function runTicket({ config, board, ticket, log }) {
  ...
  async function runEngine(cli, prompt, worktreeDir) {
    const r = await spawnEngine({
      cli, prompt, worktreeDir, runDir, model: ticket.model, tag: `impl-${cli}`,
    ...
    return { status: 'success', cli, model: ticket.model || '', commits, summary: compactAgentSummary(r.lastMessage) };
  }
  ...
  async function finalize(implemented, worktreeDir, branch, body) {
    ...
    const baseProps = {
      ...
      Model: { rich_text: [{ text: { content: modelLabel.slice(0, 200) } }] },
    };
    ...
    const reviewer = review.pickReviewer({ cli: implemented.cli, model: ticket.model || '' }, config);
    log(`reviewing "${ticket.title}" with ${reviewer.cli}${reviewer.model ? ` / ${reviewer.model}` : ''}`);
    const rev = await review.runReview({ config, board, ticket, body, worktreeDir, reviewer, log });
  ...
  try {
    ...
    for (let i = 0; i < chain.length; i++) {
      ...
      const outcome = await runEngine(cli, prompt, worktreeDir);

      if (outcome.status === 'success') {
        await finalize(outcome, worktreeDir, branch, body);
        return;
      }
      ...
      if (outcome.status === 'needs_info') {
        ...
        await notion.safeComment(
          ticket.pageId,
          `❓ The agent needs more info before it can implement this. Edit the ticket, then move it to Not started to requeue.\n\n${truncate(outcome.message, 3000)}`,
          log
        );
        return;
      }
...
module.exports = { runTicket, buildChain, QUOTA_RE, buildPrompt, extractCommitMessage, compactAgentSummary };
