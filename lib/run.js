...
- Commit ALL of your work with lowercase scoped commit messages: \`${board.scope}: <description>\`.
...
async function runTicket({ config, board, page, ticket, log }) {
  ...
  try {
    ...
    if (worktrees.isDirty(worktreeDir)) {
      worktrees.commitAll(worktreeDir, `${board.scope}: wip from ai run (uncommitted leftovers)`);
      leftoverNote = '\n\n⚠ The agent left uncommitted changes; the runner committed them as a trailing wip commit.';
    }
