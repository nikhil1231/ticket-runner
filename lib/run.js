...
function buildPrompt({ ticket, body, board, humanComments = [] }) {
  const history = Array.isArray(ticket.reviewHistory) ? ticket.reviewHistory.filter((entry) => entry && entry.notes) : [];
  const feedbackBlock = history.length
    ? `\n# IMPORTANT - reviewers have requested changes on this exact ticket before\nEvery distinct issue any reviewer has ever flagged on this ticket is listed below, oldest first, across every past attempt. Fix all of them, and do not reintroduce any of them even if this fresh attempt takes a different approach:\n${history.map((entry, index) => `${index + 1}. ${entry.notes}`).join('\n')}\n`
  ...
  return `You are completing a ticket from a tracked ticket, autonomously and non-interactively.
...
- Before starting, read project conventions such as CLAUDE.md, AGENTS.md, docs,README files when present.
...
${validationCommands}
- Do NOT run git - no commits, branches, tags,new repositories. Just make the file changes the ticket needs; the runner commits everything for you (using the \`${board.scope}:\` convention) after you finish.
- If the ticket is too vagueambiguous to implement confidently, make NO changes and end your final message with a line starting with \`NEEDS_INFO:\` followed by what needs clarifying.
- Otherwise, keep your final response under 900 characters and use exactly this structure:
  SUMMARY: onetwo sentences explaining the implemented behavior and why
  CHANGES:
...
- Do not include Markdown linksabsolute paths in the final response. Refer to files by filename only.`;
}

function stackBaseForTestingDependencies({ store, ticket, board, repo }) {
  if (!store || !ticket?.id) return null;
  const stack = store.getStack(board.key || board.app);
  const wantsIntegrationBase = ticket.meta?.bugReport?.base === 'integration';
  if (wantsIntegrationBase) {
    if (stack?.status === 'deployed' && stack.compositeSha) return stack.compositeSha;
    if (repo) {
      try { return worktrees.commitRef(repo, stack?.branch || `integration/${board.key || board.app}`); } catch {}
    }
    return null;
  }
  if (stack?.status !== 'deployed' || !stack.compositeSha) return null;

  // 1) Explicit relationships: testing-status dependenciessame-epic siblings.
  // Stack on the composite only once all of them are actually in the deployed stack.
  const related = new Map();
  for (const dep of store.dependencies(ticket.id)) {
    const blocker = store.getById(dep.dependsOnId);
    if (blocker?.status === 'testing') related.set(blocker.id, blocker);
  }
  if (ticket.parentId) {
    for (const sibling of store.childrenOf(ticket.parentId)) {
      if (sibling.id !== ticket.id && sibling.projectKey === ticket.projectKey && sibling.status === 'testing') {
        related.set(sibling.id, sibling);
      }
    }
  }
  const testingDeps = [...related.values()];
  if (testingDeps.length) {
    const inStack = new Set((stack.tickets || []).flatMap((item) => [item.shortId, item.pageId].filter(Boolean)));
    if (testingDeps.every((dep) => inStack.has(dep.shortId) || inStack.has(dep.pageId))) {
      return stack.compositeSha;
    }
  }

  // 2) File overlap: if this ticket's own changes (known from a prior round) touch
  // any file the deployed stack already changed, build on top of the stack so the
  // implementation is written against those changes instead of conflicting with
  // them during composition. First-ever implementation has no changedFiles yet, so
  // this is a no-op and the ticket branches off main (the safe default).
  if (repo) {
    const ticketFiles = store.getById(ticket.id)?.changedFiles || [];
    if (ticketFiles.length) {
      let stackFiles = [];
      try { stackFiles = worktrees.changedFilesBetween(repo, stack.baseSha, stack.compositeSha); } catch { stackFiles = []; }
      const stackSet = new Set(stackFiles);
      if (ticketFiles.some((file) => stackSet.has(file))) return stack.compositeSha;
    }
  }
...
// Decides what happens after a review verdict comes back, and applies it:
// approve -> admit to the cumulative testing stack; request changes -> either
// requeue for another implementation round or, past maxRounds, park in Needs
// info for a human. Pulled out of runTicket so it can be unit-tested without
// spinning up a real coding-agent engine.
//
// Every REQUEST_CHANGES verdict is durably appended to the ticket's
// review_history (store.appendReviewNote) *before* branching on round count.
// Previously, both the "needs a human" park and (implicitly, since nothing
// ever accumulated it) every ordinary round wiped review_feedback down to the
// latest note'', so a ticket parked for a human and later re-queued had no
// memory of what earlier reviewers had already found - it could cycle through
// the same handful of defects indefinitely. review_history is never cleared,
// and buildPrompt() now renders the full accumulated list to the implementer.
async function applyReviewOutcome({ config, store, tracker, board, ticket, implemented, branch, modelLabel, baseMirror, rev, log }) {
  ...
  if (rev.verdict === 'approve') {
    ...
    if (!['deployed', 'unchanged'].includes(deployed.status)) return { status: 'excluded' };
  ...
  if (store && ticket.id) {
    store.appendReviewNote(ticket.id, { round, reviewer: reviewerLabel, notes: rev.notes || '' });
  }
  ...
  if (rev.inconclusive || round > maxRounds) {
    ...
    // Keep reviewFeedback (do not wipe to '') so a legacy/non-store ticket - or
    // anything reading review_feedback directly - still sees the last note.
    await tracker.mirror(ticket, { ...baseMirror, status: 'needs_info', reviewRounds: 0, reviewFeedback: rev.notes || '' });
    const cycles = ticket.requeueCount || 0;
    const cycleNote = cycles >= 2
      ? `\n\nThis ticket has already bounced back to implementation ${cycles} time(s) after landing in Testingbeing parked for review. Every distinct review finding across all attempts is preserved and will be given to the next implementation, so it shouldn't need rediscovering - but consider fixing it by handsplitting the ticket if it keeps bouncing.`
      : '';
    await tracker.comment(ticket, `Needs info - ${why}.\nEngine: ${modelLabel}\nBranch: ${branch}\n\nLast review notes\n${truncate(rev.notes, 1500)}${cycleNote}`);
...
async function runTicket({ config, board, ticket, log, services = {} }) {
  ...
  async function finalize(implemented, worktreeDir, branch, body) {
    ...
    await applyReviewOutcome({ config, store, tracker, board, ticket, implemented, branch, modelLabel, baseMirror, rev, log });
  }
  ...
  try {
    ...
    const mainRef = worktrees.fetchBranch(repoPath, settings.remote, settings.mainBranch);
    ticketBaseRef = stackBaseForTestingDependencies({ store, ticket, board, repo: repoPath }) || mainRef;
    if (ticketBaseRef !== mainRef) log(`using deployed testing stack ${ticketBaseRef.slice(0, 7)} as implementation base`);
...
module.exports = { runTicket, QUOTA_RE, buildPrompt, extractCommitMessage, compactAgentSummary, applyReviewOutcome, stackBaseForTestingDependencies };
