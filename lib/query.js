...
function buildQueryPrompt({ ticket, body, board, humanComments = [] }) {
  ...
  const comments = humanComments.length
    ...
    : '(none)';
  return `You are answering a query-only tracked ticket. Work autonomously and non-interactively.
...
- Read project conventions such as CLAUDE.md, AGENTS.md, docs,README files when present if they affect the answer.
- Answer the question directly and include concise evidence from the codeticket context.
...
async function appendQueryAnswer(tracker, ticket, parsed) {
  return tracker.appendSection(ticket, { markdown: formatQueryAnswer(parsed) });
}
...
function queryRunConfig(config) {
  const codexConfig = (config.adapters && config.adapters.codex) || {};
  return {
    ...config,
    adapters: {
      ...(config.adapters || {}),
...
async function runQueryTicket({
  ...
  if (parsed.status === 'needs_info') {
    await appendQueryAnswer(tracker, ticket, {
      answer: 'I need one detail before I can answer this confidently.',
      followUp: parsed.followUp,
    });
  } else {
    await appendQueryAnswer(tracker, ticket, parsed);
  }

  const modelLabel = label(fallback.candidate);
  await tracker.mirror(ticket, { status: 'needs_info', attempts: ticket.attempts, lastAgent: modelLabel });
  await tracker.comment(
    ticket,
    parsed.followUp
