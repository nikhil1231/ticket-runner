...
- You MAY read other files in \`${board.appDir}\` for context. Do NOT modify files and do NOT run git.
...
async function runReview({ config, board, ticket, body, worktreeDir, implementer, log }) {
  ...
  const reviewPatch = worktrees.diff(worktreeDir, config.baseBranch);
