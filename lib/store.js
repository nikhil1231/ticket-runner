...
const TRANSITIONS = {
  queued: ['in_progress'],
  in_progress: ['testing', 'in_review', 'needs_info', 'queued', 'failed'],
  testing: ['done', 'in_review', 'queued'],
  in_review: ['queued', 'testing', 'done'],
  needs_info: ['queued'],
  done: ['queued'],
  failed: ['queued'],
  cancelled: ['queued'],
...
function rowToTicket(row) {
  ...
  return {
    ...
    trackerMeta: parse('tracker_meta', {}),
...
function createStore({ baseDir, db, now = () => new Date().toISOString() }) {
  ...
  return {
    // identity & ingest
    getById, getByShortId, getByTrackerId, upsertFromTracker, refreshIntent,
    // state machine
    ...
    // hierarchy & deps
    setParent, addDependency, removeDependency, dependencies,
    // stacks / repairs / kv
