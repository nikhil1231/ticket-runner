...
const TRANSITIONS = {
  ...
  in_progress: ['testing', 'in_review', 'needs_info', 'queued', 'failed', 'cancelled'],
  testing: ['done', 'in_review', 'needs_info', 'queued', 'cancelled'],
  in_review: ['queued', 'testing', 'done', 'needs_info', 'cancelled'],
  needs_info: ['queued', 'cancelled'],
  done: ['queued'],
  failed: ['queued'],
  cancelled: ['queued'],
};
...
function createStore({ baseDir, db, now = () => new Date().toISOString() }) {
  ...
  function transition(id, toStatus, patch = {}) {
    return tx(() => {
      ...
      db.prepare('UPDATE tickets SET status = ?, closed_at = ?, updated_at = ? WHERE id = ?')
  ...
  function upsertFromTracker(input) {
    const {
      tracker, trackerId, projectKey, kind = 'feature', title, createdAt,
      enginePin = '', modelPin = '', trackerMeta = {}, mirroredStatus = null,
      status = 'queued', shortId,
    } = input;
    ...
    return tx(() => {
      ...
      const ts = now();
      if (existing) {
        db.prepare(`UPDATE tickets SET title = ?, kind = ?, engine_pin = ?, model_pin = ?,
                    tracker_meta = ?, project_key = ?, mirrored_status = COALESCE(?, mirrored_status), updated_at = ?
                    WHERE id = ?`)
          .run(title ?? existing.title, kind, enginePin, modelPin, JSON.stringify(trackerMeta),
            projectKey ?? existing.projectKey, mirroredStatus, ts, existing.id);
        return getById(existing.id);
      }
      const sid = shortId || deriveShortId(tracker, trackerId);
      db.prepare(`INSERT INTO tickets(short_id, project_key, kind, title, status, engine_pin, model_pin,
                  tracker, tracker_id, tracker_meta, mirrored_status, created_at, updated_at)
                  VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)`)
        .run(sid, projectKey, kind, title || '(untitled)', status, enginePin, modelPin,
  ...
  function refreshIntent(id, { title, body, enginePin, modelPin } = {}) {
    const sets = [];
    const values = [];
  ...
  function readyTickets({ projectKey, kinds = CLAIMABLE_KINDS } = {}) {
    ...
    sql += ' ORDER BY t.created_at, t.short_id';
  ...
  function createLocalTicket({ projectKey, kind, title, body = '', parentId = null, status = 'queued', tracker, trackerMeta = {}, meta = {} }) {
    ...
    return tx(() => {
      ...
      const ts = now();
      db.prepare(`INSERT INTO tickets(short_id, project_key, kind, parent_id, title, body, status,
                  tracker, tracker_id, tracker_meta, meta, created_at, updated_at)
                  VALUES(?,?,?,?,?,?,?,?,NULL,?,?,?,?)`)
        .run(shortId, projectKey, kind, parentId, title || '(untitled)', body, status,
  ...
  return {
    // identity & ingest
    getById, getByShortId, getByTrackerId, upsertFromTracker, createLocalTicket, refreshIntent, retargetTracker,
    // state machine
    transition, claimNext, readyTickets, listByStatus,
    // worktree lifecycle
    recordWorktree, recordImplementation, clearWorktree, markCommentsProcessed, setPendingComments,
    // outbound side-effects
...
module.exports = { createStore, deriveShortId, canonical, STATUSES, TRANSITIONS, assertTransition };
