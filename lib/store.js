...
// own board vocabulary. Kept here so the state machine is tracker-agnostic.
...
const TRANSITIONS = {
  ...
  // are all already terminal (mergedcancelled), so there is nothing to park
...
// Columns that may be set through transition()'s patchupdateFields, mapped
...
function createStore({ baseDir, db, now = () => new Date().toISOString() }) {
  ...
  function fairReadyTickets({ projectKey } = {}) {
    const ready = readyTickets({ projectKey });
    if (projectKey || ready.length < 2) return ready;
    const priority = ready[0].priority;
    const top = ready.filter((ticket) => ticket.priority === priority);
    const rest = ready.filter((ticket) => ticket.priority !== priority);
    const projects = [];
    for (const ticket of top) {
      const key = ticket.projectKey || '';
      if (!projects.includes(key)) projects.push(key);
    }
    if (projects.length < 2) return ready;
    const lastProject = getKv(`claim:last-project:${priority}`, null);
    const lastIndex = projects.indexOf(lastProject);
    const nextProject = projects[(lastIndex + 1) % projects.length];
    const groupedTop = [
      ...top.filter((ticket) => (ticket.projectKey || '') === nextProject),
      ...top.filter((ticket) => (ticket.projectKey || '') !== nextProject),
    ];
    return [...groupedTop, ...rest];
  }

  function claimTicket(id, { recordRotation = true } = {}) {
    return tx(() => {
      const ready = readyTickets();
      const ticket = ready.find((item) => item.id === id);
      if (!ticket) return null;
      const claimed = transition(ticket.id, 'in_progress', { attempts: ticket.attempts + 1 });
      if (recordRotation) setKv(`claim:last-project:${claimed.priority}`, claimed.projectKey || '');
      return claimed;
    });
  }
  ...
  function claimNext({ projectKey } = {}) {
    return tx(() => {
      const ready = fairReadyTickets({ projectKey });
      const first = ready[0];
  ...
  // `github:<owner>/<repo>`) so that once the outbound mirror op
  ...
  return {
    ...
    // state machine
    transition, claimNext, claimTicket, readyTickets, fairReadyTickets, listByStatus, listArchivable, archiveTicket, appendReviewNote,
    // worktree lifecycle
