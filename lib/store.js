...
function createStore({ baseDir, db, now = () => new Date().toISOString() }) {
  ...
  function upsertFromTracker(input) {
    ...
    return tx(() => {
      ...
      db.prepare(`INSERT INTO tickets(short_id, project_key, kind, title, body, status, engine_pin, model_pin,
                  tracker, tracker_id, tracker_meta, mirrored_status, created_at, updated_at)
                  VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)`)
        .run(sid, projectKey, kind, title || '(untitled)', body || '', status, enginePin, modelPin,
