...
async function spawnEngine({ cli, prompt, worktreeDir, runDir, model, tag, config, timeoutMs, log }) {
  ...
  // Adapters whose output is structured (e.g. claude's JSON) parse it here to
  // recover the assistant message and token usage; a parse failure leaves both
  // unset so the raw-stdout fallback below still yields something to classify.
  let usage = null;
  if (typeof spec.parseResult === 'function') {
    const parsed = spec.parseResult({
      stdout: fs.existsSync(outFile) ? fs.readFileSync(outFile, 'utf8') : '',
      stderr: fs.existsSync(errFile) ? fs.readFileSync(errFile, 'utf8') : '',
    }) || {};
    if (typeof parsed.lastMessage === 'string' && parsed.lastMessage.trim()) lastMessage = parsed.lastMessage.trim();
    if (parsed.usage) usage = parsed.usage;
  }
  if (!lastMessage) lastMessage = readTail(outFile, 30);
  const quota = QUOTA_RE.test(`${readTail(errFile, 80)}\n${readTail(outFile, 80)}`);

  return { ...result, lastMessage, quota, usage, outFile, errFile };
}
