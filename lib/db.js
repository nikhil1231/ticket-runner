...
const MIGRATIONS = [
  ...
  `,
  // 3: human-owned ticket priority. Missing or invalid tracker values are
  // normalized by the store to Medium, and queue claiming sorts by this before
  // the existing oldest-first order.
  `
  ALTER TABLE tickets ADD COLUMN priority TEXT NOT NULL DEFAULT 'Medium';
  CREATE INDEX tickets_claim_priority ON tickets(status, priority, created_at);
  `,
];
