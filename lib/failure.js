...
const CONFIG_RE = /GITHUB_TOKEN|EXPO_TOKEN|authentication|unauthorized|forbidden|permission denied|not found in PATH|ENOENT|unknown (?:provider|adapter|model)|GitHub .+ -> (?:400|401|403|404)/i;
const PROVIDER_RE = /usage limit|rate.?limit|quota|status 429|timed out|exited with code|spawn error|model.+not found/i;
const TRANSIENT_RE = /ECONNRESET|ECONNREFUSED|ETIMEDOUT|EAI_AGAIN|socket hang up|HTTP 5\d\d|GitHub .+ -> 5\d\d|index\.lock|could not resolve host/i;
const INFRA_RE = /runner error|git .+(?:failed|fatal)|worktree|rev-parse|reset --hard|yarn install|npm install|GitHub (?:GET|POST|PATCH)|EACCES|ENOSPC|corrupt/i;
